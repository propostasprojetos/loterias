...
$startIndex = $content.IndexOf("// ===== LOTOFÁCIL — Core Validation (unchanged) =====")
$endIndex = $content.IndexOf("// ===== HISTORY (localStorage) =====")

if ($startIndex -ne -1 -and $endIndex -ne -1) {
    $content = $content.Substring(0, $startIndex) + $engineContent + "`n" + $content.Substring($endIndex)
}

$content = $content.Replace("const currentView = document.querySelector('[id^=`"view-`"]:not(.hidden)');", "await loadAvailableGames();`n                const currentView = document.querySelector('[id^=`"view-`"]:not(.hidden)');")

$rbStart = $content.IndexOf("function registerBetFromGenerator()")
if ($rbStart -ne -1) {
    $rbEnd = $content.IndexOf("}", $rbStart)
    if ($rbEnd -ne -1) {
        $content = $content.Substring(0, $rbStart) + $newRB + $content.Substring($rbEnd + 1)
    }
}

$content = $content.Replace("lf: currentLF,", "games: currentGamesData,")
$content = $content.Replace("qn: currentQN", "")
$content = $content.Replace("currentLF = entry.lf || [];", "currentGamesData = entry.games || {};")
$content = $content.Replace("currentQN = entry.qn || [];", "")
$content = $content.Replace("selectedLF.clear();", "")
$content = $content.Replace("selectedQN.clear();", "")
$content = $content.Replace("if (!currentLF.length && !currentQN.length) return;", "")

$content = $content.Replace("let currentLF = [];", "")
$content = $content.Replace("let currentQN = [];", "")
$content = $content.Replace("let selectedLF = new Set();", "")
$content = $content.Replace("let selectedQN = new Set();", "")
$content = $content.Replace("let currentBaseSetLF = null;", "")

Set-Content -Path "app.js" -Value $content -Encoding UTF8
Write-Output "app.js refactored successfully via Powershell."
